...
cp "$BASE/j1msky-work-feed.service" ~/.config/systemd/user/
...
systemctl --user enable --now j1msky-work-feed.service

echo "Enabled watchdog services:"
systemctl --user --no-pager --type=service | grep -E 'j1msky-(agency|alexa-bridge|alexa-center|work-feed)' || true
